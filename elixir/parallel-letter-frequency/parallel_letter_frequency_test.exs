...
defmodule FrequencyTest do
  ...
  test "slice single text into equal parts - text length equal to number of part" do
    text = "abcd"
    parts = Frequency.slice([text], 4)
    assert parts == ['d', 'c', 'b', 'a']
  end


  test "slice single text into equal parts - text length greater than the number of part" do
    text = "abcdef"
    parts = Frequency.slice([text], 4)
    assert parts == ['f', 'e', 'dc', 'ba']
  end

  test "slice single text into equal parts - text length smaller than the number of part" do
    text = "abc"
    parts = Frequency.slice([text], 4)
    assert parts == ['c', 'b', 'a']
  end

  test "slice multiple texts into equal parts" do
    texts = ["abcdef", "ghijklm", "n"]
    parts = Frequency.slice(texts, 4)
    assert parts == ['n', 'm', 'lk', 'ji', 'hg', 'f', 'e', 'dc', 'ba']
  end

  test "count_letters" do
    stats = Frequency.count_letters('aaabbe', %{})
    assert stats == %{"a"=> 3, "b"=>2, "e"=>1}
  end
end
